...
# 従来手法の予測結果の算出
# 入力（説明変数:df，目的変数:df，プロジェクト名:str）
# 出力（プロジェクトごとの予測結果（適合率，再現率，F1値，正解率）:df）
def predict(explanatory_variable, label, project_name):
  # モデルの初期化
...
def create_all_model(cnum):
  ...
  model_all = LogisticRegression(penalty='l2',          # 正則化項(L1正則化 or L2正則化が選択可能)
                                 class_weight='balanced',  # クラスに付与された重み
                                 random_state=0,     # 乱数シード
                                 solver='lbfgs',        # ハイパーパラメータ探索アルゴリズム
                                 max_iter=10000,          # 最大イテレーション数
...
def create_model(cnum):
  ...
  model_dict = {}
  for i in range(cnum):
    model_dict["cluster_"+str(i)] = LogisticRegression(penalty='l2',          # 正則化項(L1正則化 or L2正則化が選択可能)
                                       class_weight='balanced',  # クラスに付与された重み
                                       random_state=0,     # 乱数シード
                                       solver='lbfgs',        # ハイパーパラメータ探索アルゴリズム
                                       max_iter=10000,          # 最大イテレーション数
