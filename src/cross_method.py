...
for project_name in project_list:
    ...
    result = {
        "precision": format(
            precision_score(ans_list, predict_result, zero_division=np.nan), ".2f"
        ),
        "recall": format(
            recall_score(ans_list, predict_result, zero_division=np.nan), ".2f"
        ),
        "f1_score": format(
            f1_score(ans_list, predict_result, zero_division=np.nan), ".2f"
        ),
        "accuracy": format(accuracy_score(ans_list, predict_result), ".2f"),
    }
    # print(result)
    result_df = pd.concat(
        [result_df, pd.DataFrame([result], index=[project_name])], axis=0
    )
